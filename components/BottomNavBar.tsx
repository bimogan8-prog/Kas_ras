...
const NavItem = ({ icon, label, isActive = false }: { icon: React.ReactNode; label: string; isActive?: boolean }) => (
    <motion.button whileTap={{scale: 0.9}} className={`flex flex-col items-center gap-1 transition-colors ${isActive ? 'text-indigo-400' : 'text-gray-400 hover:text-white'}`}>
        {icon}
        <span className="text-xs">{label}</span>
    </motion.button>
);

export const BottomNavBar: React.FC<BottomNavBarProps> = ({ onAddClick }) => {
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 p-2">
        <div className="bg-slate-900/70 backdrop-blur-xl border border-white/10 max-w-lg mx-auto flex justify-around items-center p-2 rounded-2xl shadow-lg">
            <NavItem icon={<HomeIcon className="w-6 h-6"/>} label="Beranda" isActive={true} />
            ...
            <NavItem icon={<div className="w-6 h-6" />} label="" />
        </div>
    </div>
  );
};
