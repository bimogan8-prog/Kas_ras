...
export const TransactionItem: React.FC<TransactionItemProps> = ({ transaction, onEdit, onDelete, onPreviewImage }) => {
  ...
  return (
    <motion.li variants={itemVariants} layout>
      <GlassCard 
        className={`p-3 flex flex-col relative overflow-hidden ${hasAttachment ? 'cursor-pointer' : ''}`}
        ...
        <div className="absolute top-2 right-2 z-10">
          ...
          {menuOpen && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-full right-0 mt-1 w-28 bg-gray-900/[0.8] backdrop-blur-xl border border-white/[0.1] rounded-lg shadow-xl z-20"
